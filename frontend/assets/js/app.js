...
function navigate(view){
  if(typeof clearVerifiedVideoTracker==='function') clearVerifiedVideoTracker();
...
function loadStudentDashboard(){
  serverCall('getStudentDashboardData',[AppState.user.id],{
    loading:'Menyiapkan dashboard belajar...',
    success:function(res){
      if(AppState.view!=='dashboard') return;
      if(!res || !res.success){
        showToast(res&&res.message?res.message:'Gagal memuat dashboard siswa.','danger');
        return;
      }
      ...
      var cont=data.continue_course||null;
      var achievement=data.achievement_highlight||null;
      var firstName=String((data.student&&data.student.name)||AppState.user.name||'Siswa').trim().split(/\s+/)[0]||'Siswa';

      var continueHtml='';
      if(cont&&cont.course){
        var lesson=cont.current_lesson||{};
        var stage=lesson.completed?'Selesai':flowStageLabel(lesson.next_stage);
        continueHtml=
          '<div class="student-continue-card h-100">'+
            '<div class="d-flex justify-content-between align-items-start gap-3">'+
              '<div>'+
                '<div class="small fw-semibold text-primary mb-1"><i class="bi bi-play-circle-fill me-1"></i>LANJUTKAN BELAJAR</div>'+
                '<h4 class="fw-bold mb-1">'+esc(cont.course.title)+'</h4>'+
                '<div class="text-secondary">'+
                  (lesson.title
                    ? 'Lesson '+Number(lesson.order||1)+' · '+esc(lesson.title)
                    : 'Course sudah selesai')+
                '</div>'+
              '</div>'+
              '<span class="badge text-bg-light">'+Number(cont.progress_percent||0)+'%</span>'+
            '</div>'+
            '<div class="progress my-3" style="height:9px"><div class="progress-bar" style="width:'+Number(cont.progress_percent||0)+'%"></div></div>'+
            '<div class="d-flex flex-wrap justify-content-between align-items-center gap-2">'+
              '<div class="small text-secondary">'+
                (lesson.title
                  ? '<i class="bi bi-signpost-split me-1"></i>Tahap berikutnya: <strong>'+esc(stage)+'</strong>'
                  : '<i class="bi bi-trophy me-1"></i>Semua Lesson selesai')+
              '</div>'+
              '<button class="btn btn-primary" onclick="openCourseFromDashboard(\''+escAttr(cont.course.id)+'\')">'+
                '<i class="bi bi-arrow-right-circle me-1"></i>'+(cont.course_complete?'Lihat Course':'Lanjutkan')+
              '</button>'+
            '</div>'+
          '</div>';
      }else{
        continueHtml=
          '<div class="student-continue-card h-100">'+
            '<div class="text-center py-4">'+
              ...
              '<h5 class="mt-3">Belum ada Course aktif</h5>'+
              '<button class="btn btn-outline-primary mt-2" onclick="navigate(\'courses\')">Buka Katalog</button>'+
            '</div>'+
          '</div>';
      }
      ...
      if(achievement){
        achievementHtml=
          '<div class="student-highlight-card h-100">'+
            '<div class="student-highlight-icon"><i class="bi bi-award-fill"></i></div>'+
            '<div class="small text-secondary mt-3">Achievement Highlight</div>'+
            '<h5 class="fw-bold mb-1">'+esc(achievement.title)+'</h5>'+
            '<div class="small text-secondary mb-3">'+esc(achievement.course_title)+'</div>'+
            '<button class="btn btn-sm btn-outline-primary" onclick="openCourseFromDashboard(\''+escAttr(achievement.course_id)+'\')">Lihat Achievement</button>'+
          '</div>';
      }else{
        achievementHtml=
          '<div class="student-highlight-card h-100">'+
            '<div class="student-highlight-icon"><i class="bi bi-stars"></i></div>'+
            '<div class="small text-secondary mt-3">Achievement</div>'+
            '<h5 class="fw-bold mb-1">Badge pertama menunggu</h5>'+
            '<div class="small text-secondary">Selesaikan Lesson pertama untuk membuka achievement.</div>'+
          '</div>';
      }
      ...
      if(cont){
        targetHtml=
          '<div class="student-target-card">'+
            '<div class="d-flex gap-3 align-items-start">'+
              '<div class="student-target-icon"><i class="bi bi-signpost-2-fill"></i></div>'+
              '<div><div class="small text-secondary">Target Berikutnya</div><div class="fw-semibold">'+esc(cont.next_goal||'Lanjutkan progress Course Anda.')+'</div></div>'+
            '</div>'+
          '</div>';
      }
      ...
      courses.forEach(function(c){
        var lessonText=Number(c.completed_lessons||0)+' / '+Number(c.total_lessons||0)+' Lesson selesai';
        var scoreText=c.course_score===null||c.course_score===undefined
          ? 'Belum dinilai'
          : 'Nilai '+Number(c.course_score||0);

        courseRows+=
          '<div class="col-12 col-md-6 col-xl-4">'+
            '<div class="student-course-summary h-100" onclick="openCourseFromDashboard(\''+escAttr(c.course.id)+'\')">'+
              '<div class="d-flex justify-content-between gap-2 mb-2">'+
                '<div class="fw-bold">'+esc(c.course.title)+'</div>'+
                (c.course_complete
                  ? '<span class="badge text-bg-success">Selesai</span>'
                  : '<span class="badge text-bg-light">'+Number(c.progress_percent||0)+'%</span>')+
              '</div>'+
              '<div class="small text-secondary mb-2">'+esc(lessonText)+'</div>'+
              '<div class="progress mb-3" style="height:7px"><div class="progress-bar" style="width:'+Number(c.progress_percent||0)+'%"></div></div>'+
              '<div class="d-flex justify-content-between small">'+
                '<span><i class="bi bi-award me-1"></i>'+Number(c.badge_count||0)+'/'+Number(c.badge_capacity||6)+' badge</span>'+
      ...
      if(!courseRows){
        courseRows='<div class="col-12"><div class="text-center text-secondary py-4">Belum ada Course untuk ditampilkan.</div></div>';
      }

      document.getElementById('content').innerHTML=
        '<div class="student-dashboard-hero mb-4">'+
          '<div>'+
            '<div class="small opacity-75 mb-1">DASHBOARD BELAJAR</div>'+
            '<h2 class="fw-bold mb-1">Selamat datang, '+esc(firstName)+' 👋</h2>'+
            '<div class="opacity-75">Lanjutkan progres belajar Anda dari tempat terakhir.</div>'+
          '</div>'+
          '<div class="student-dashboard-hero-icon"><i class="bi bi-mortarboard-fill"></i></div>'+
        '</div>'+

        '<div class="row g-3 mb-4">'+
          studentDashboardStat('Course Aktif',Number(stats.active_courses||0),'bi-collection-play')+
          studentDashboardStat('Lesson Selesai',Number(stats.completed_lessons||0),'bi-check2-circle')+
          studentDashboardStat('Progress Rata-rata',Number(stats.average_progress||0)+'%','bi-graph-up-arrow')+
          studentDashboardStat('Badge Terbuka',Number(stats.badges_unlocked||0),'bi-award-fill')+
        '</div>'+

        '<div class="row g-3 mb-4">'+
          '<div class="col-12 col-xl-8">'+continueHtml+'</div>'+
          '<div class="col-12 col-xl-4">'+achievementHtml+'</div>'+
        '</div>'+

        (targetHtml?'<div class="mb-4">'+targetHtml+'</div>':'')+

        '<div class="panel">'+
          '<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">'+
            '<div><h5 class="mb-1"><i class="bi bi-grid-1x2-fill me-2 text-primary"></i>Ringkasan Course</h5><div class="small text-secondary">Pantau progres, badge, dan nilai setiap Course.</div></div>'+
            '<button class="btn btn-outline-primary btn-sm" onclick="navigate(\'courses\')">Buka Katalog</button>'+
          '</div>'+
          '<div class="row g-3">'+courseRows+'</div>'+
        '</div>';
    },
    failure:function(err){
      showToast(errorMessage(err),'danger');
...
function studentDashboardStat(label,value,icon){
  return '<div class="col-6 col-xl-3">'+
    '<div class="student-dashboard-stat h-100">'+
      '<div class="d-flex justify-content-between align-items-start">'+
        '<div><div class="small text-secondary">'+esc(label)+'</div><div class="fs-2 fw-bold mt-1">'+esc(value)+'</div></div>'+
        '<div class="student-dashboard-stat-icon"><i class="bi '+icon+'"></i></div>'+
      '</div>'+
    '</div>'+
  '</div>';
}

function openCourseFromDashboard(courseId){
  AppState.view='courses';
  closeSidebar();

  var btns=document.querySelectorAll('.nav-btn');
  for(var i=0;i<btns.length;i++){
    if(btns[i].getAttribute('data-view')==='courses') btns[i].classList.add('active');
...
function renderClassroom(){
  ...
  if(AppState.selectedLesson){
    for(var i=0;i<lessons.length;i++){
      if(String(lessons[i].id)===String(AppState.selectedLesson.id)){
        foundSelected=lessons[i];
        break;
      }
    }
  }

  // Jika Lesson yang sebelumnya dipilih sekarang terkunci, otomatis pilih
  // Lesson pertama yang masih terbuka dan belum selesai.
  if(!foundSelected || foundSelected.locked){
    foundSelected=null;
    for(var u=0;u<lessons.length;u++){
      if(!lessons[u].locked && !lessons[u].completed){
        foundSelected=lessons[u];
        break;
      }
    }
    if(!foundSelected){
      for(var u2=lessons.length-1;u2>=0;u2--){
        if(!lessons[u2].locked){
          foundSelected=lessons[u2];
          break;
        }
      }
    }
  ...
  AppState.selectedLesson=foundSelected||(lessons[0]||null);

  document.getElementById('pageTitle').textContent=data.course.title;
  document.getElementById('pageSubtitle').textContent='Ruang Belajar · Alur Bertahap';
  ...
  if(lessons.length){
    ...
    for(var j=0;j<lessons.length;j++){
      ...
      var statusIcon='';
      if(l.completed){
        statusIcon='<i class="bi bi-check-circle-fill text-success" title="Lesson selesai"></i>';
      }else if(l.locked){
        statusIcon='<i class="bi bi-lock-fill text-secondary" title="Lesson terkunci"></i>';
      }else{
        statusIcon='<i class="bi bi-play-circle text-primary" title="Lesson aktif"></i>';
      }

      var stageLabel=l.completed?'Selesai':flowStageLabel(l.next_stage);
      var lessonPct=Math.max(0,Math.min(100,Number(l.flow_progress_percent||0)));
      var buttonAttrs=l.locked
        ? ' disabled aria-disabled="true"'
        : ' onclick="selectLesson(\''+escAttr(l.id)+'\',this)"';

      larr.push(
        '<button class="lesson-item '+activeClass+'"'+buttonAttrs+'>'+
          '<div class="d-flex justify-content-between align-items-center">'+
            '<div class="small text-secondary">Lesson '+(j+1)+' · '+esc(stageLabel)+'</div>'+
            statusIcon+
          '</div>'+
          '<div class="fw-semibold">'+esc(l.title)+'</div>'+
          '<div class="d-flex justify-content-between small mt-2"><span>Progress</span><strong>'+lessonPct+'%</strong></div>'+
          '<div class="progress mt-1" style="height:6px"><div class="progress-bar" style="width:'+lessonPct+'%"></div></div>'+
          (l.locked?'<div class="small text-secondary mt-1"><i class="bi bi-lock me-1"></i>Selesaikan Lesson sebelumnya</div>':'')+
        '</button>'
      );
    }
  ...
  document.getElementById('content').innerHTML=
    ...
    '<div class="alert alert-light border d-flex align-items-center gap-2 mb-3">'+
      ...
      '<div><strong>Alur Lesson:</strong> Video → Materi Teks → Quiz → Tugas → Lesson berikutnya.</div>'+
    '</div>'+
    renderCourseGamification(data)+
    '<div class="classroom">'+
      '<aside class="lesson-sidebar">'+
        '<div class="p-3 border-bottom">'+
          '<div class="fw-bold">'+esc(data.course.title)+'</div>'+
          '<div class="small text-secondary mb-2">'+lessons.length+' Lesson</div>'+
          '<div class="d-flex justify-content-between small mb-1"><span>Progress Course</span><strong>'+Number((data.learning_flow||{}).course_progress_percent||0)+'%</strong></div>'+
          '<div class="progress" style="height:7px"><div class="progress-bar" style="width:'+Number((data.learning_flow||{}).course_progress_percent||0)+'%"></div></div>'+
        '</div>'+
...
function flowStageLabel(stage){
  ...
  return map[String(stage||'')]||'Belajar';
}

function formatLessonPoint(value){
  var n=Number(value||0);
  if(Math.abs(n-Math.round(n))<0.001)return String(Math.round(n));
  return n.toFixed(1);
}

function renderLessonProgressSummary(l){
  var s=l.lesson_score||{};
  var flowPct=Math.max(0,Math.min(100,Number(l.flow_progress_percent||0)));
  var hasAssessment=!!s.has_assessment;
  var score=s.total===null||s.total===undefined?'':Number(s.total||0);
  var scoreLabel=!hasAssessment
    ? 'Tidak Dinilai'
    : (s.grade_ready?'Nilai Akhir':'Nilai Sementara');

  function componentText(hasComponent, completed, points, max, waitingText){
    if(!hasComponent){
      return '<span class="text-secondary"><i class="bi bi-dash-circle me-1"></i>Tidak ada</span>';
    }
    if(waitingText){
      return '<span class="text-warning"><i class="bi bi-clock-history me-1"></i>'+esc(waitingText)+' · '+formatLessonPoint(points)+'/'+formatLessonPoint(max)+'</span>';
    }
    if(completed){
      return '<span class="text-success"><i class="bi bi-check-circle-fill me-1"></i>'+formatLessonPoint(points)+'/'+formatLessonPoint(max)+'</span>';
    }
    return '<span class="text-secondary">'+formatLessonPoint(points)+'/'+formatLessonPoint(max)+'</span>';
  }

  var videoText=componentText(
    !!l.has_video,
    !!l.video_completed,
    s.video_points,
    s.video_max
  ...
  var textText=l.has_text
    ? (l.text_completed
        ? '<span class="text-success"><i class="bi bi-check-circle-fill me-1"></i>Selesai</span>'
        : '<span class="text-secondary"><i class="bi bi-hourglass-split me-1"></i>Belum</span>')
    : '<span class="text-secondary"><i class="bi bi-dash-circle me-1"></i>Tidak ada</span>';

  var quizText=componentText(
    !!l.has_quiz,
    !!l.quiz_completed,
    s.quiz_points,
    s.quiz_max
  ...
  var taskWaiting=l.has_task &&
    l.task_completed &&
    Number(s.graded_task_count||0)<Number(s.task_count||0);

  var taskText=componentText(
    !!l.has_task,
    !!l.task_completed && !taskWaiting,
    s.task_points,
    s.task_max,
    taskWaiting?'Menunggu nilai':''
  );

  var weightNote='';
  if(hasAssessment){
    var parts=[];
    if(l.has_video)parts.push('Video '+formatLessonPoint(s.video_max)+'%');
    if(l.has_quiz)parts.push('Quiz '+formatLessonPoint(s.quiz_max)+'%');
    if(l.has_task)parts.push('Tugas '+formatLessonPoint(s.task_max)+'%');
    weightNote='<div class="small text-secondary mt-3"><i class="bi bi-sliders me-1"></i>Bobot adaptif: '+parts.join(' · ')+'</div>';
  }

  return '<div class="panel mb-4">'+
    '<div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">'+
      '<div>'+
        '<div class="small text-secondary">Progress Lesson</div>'+
        '<div class="d-flex align-items-baseline gap-2"><div class="display-6 fw-bold">'+flowPct+'%</div><div class="text-secondary">alur belajar</div></div>'+
      '</div>'+
      '<div class="text-end">'+
        '<div class="small text-secondary">'+scoreLabel+'</div>'+
        '<div class="display-6 fw-bold">'+(hasAssessment?Math.round(score)+'/100':'—')+'</div>'+
      '</div>'+
    '</div>'+
    '<div class="progress mb-3" style="height:10px"><div class="progress-bar" style="width:'+flowPct+'%"></div></div>'+
    '<div class="row g-2">'+
      '<div class="col-6 col-xl-3"><div class="border rounded-3 p-3 h-100"><div class="small text-secondary mb-1"><i class="bi bi-play-btn me-1"></i>Video</div><div class="fw-semibold">'+videoText+'</div></div></div>'+
      '<div class="col-6 col-xl-3"><div class="border rounded-3 p-3 h-100"><div class="small text-secondary mb-1"><i class="bi bi-file-text me-1"></i>Materi</div><div class="fw-semibold">'+textText+'</div></div></div>'+
      '<div class="col-6 col-xl-3"><div class="border rounded-3 p-3 h-100"><div class="small text-secondary mb-1"><i class="bi bi-patch-question me-1"></i>Quiz</div><div class="fw-semibold">'+quizText+'</div></div></div>'+
      '<div class="col-6 col-xl-3"><div class="border rounded-3 p-3 h-100"><div class="small text-secondary mb-1"><i class="bi bi-clipboard-check me-1"></i>Tugas</div><div class="fw-semibold">'+taskText+'</div></div></div>'+
    '</div>'+
    weightNote+
    (!hasAssessment
      ? '<div class="small text-secondary mt-3"><i class="bi bi-info-circle me-1"></i>Lesson ini tidak memiliki komponen penilaian. Progress belajar tetap dicatat, tetapi tidak memengaruhi Nilai Course.</div>'
      : (!s.grade_ready && l.task_completed && l.has_task
          ? '<div class="small text-secondary mt-3"><i class="bi bi-info-circle me-1"></i>Lesson sudah selesai untuk alur belajar, tetapi Nilai Akhir akan diperbarui setelah Guru menilai seluruh tugas.</div>'
          : ''))+
  '</div>';
}
...
/* ============================================================
   GAMIFICATION LITE V1.6
   Badge & celebration TIDAK memengaruhi nilai akademik.
   ============================================================ */

function getCourseGamificationState(data){
  data=data||{};
  var lessons=Array.isArray(data.lessons)?data.lessons:[];
  var flow=data.learning_flow||{};

  var completeCount=0;
  var videoDoneCount=0;
  var quizDoneCount=0;
  var taskDoneCount=0;
  var quizMasterCount=0;

  lessons.forEach(function(l){
    if(l.completed) completeCount++;
    if(l.has_video && l.video_completed) videoDoneCount++;
    if(l.has_quiz && l.quiz_completed) quizDoneCount++;
    if(l.has_task && l.task_completed) taskDoneCount++;

    var s=l.lesson_score||{};
    var quizBest=Number(s.quiz_best||0);
    if(l.has_quiz && l.quiz_completed && quizBest>=90) quizMasterCount++;
  });

  var total=lessons.length;
  var courseComplete=total>0 && completeCount===total;
  var courseScore=flow.course_score===null||flow.course_score===undefined
    ? null
    : Number(flow.course_score||0);
  var mastery=courseComplete &&
    !!flow.course_grade_ready &&
    courseScore!==null &&
    courseScore>=85;
  ...
  var badges=[
    {
      ...
      desc:'Selesaikan 1 Lesson.',
      unlocked:completeCount>=1
    },
    {
      id:'video_finisher',
      icon:'bi-play-btn-fill',
      title:'Video Finisher',
      desc:'Selesaikan minimal 1 video terverifikasi.',
      unlocked:videoDoneCount>=1
    },
    {
      id:'quiz_master',
      icon:'bi-bullseye',
      title:'Quiz Master',
      desc:'Raih nilai Quiz minimal 90.',
      unlocked:quizMasterCount>=1
    },
    {
      id:'learning_momentum',
      icon:'bi-rocket-takeoff-fill',
      title:'Learning Momentum',
      desc:'Selesaikan 3 Lesson dalam Course.',
      unlocked:completeCount>=3
    },
    {
      id:'course_finisher',
      icon:'bi-trophy-fill',
      title:'Course Finisher',
      desc:'Selesaikan seluruh Lesson.',
    ...
    },
    {
      ...
      desc:'Selesaikan Course dengan nilai akhir minimal 85.',
      unlocked:mastery
    }
  ];
  ...
  var nextGoal='';
  if(total===0){
    nextGoal='Course belum memiliki Lesson.';
  }else if(!courseComplete){
    var remaining=Math.max(0,total-completeCount);
    nextGoal=remaining===1
      ? '1 Lesson lagi menuju badge Course Finisher.'
      : remaining+' Lesson lagi menuju badge Course Finisher.';
  }else if(!mastery && flow.course_grade_ready){
    nextGoal='Course selesai. Tingkatkan nilai Course hingga 85 untuk badge Course Mastery.';
  }else if(!mastery){
    nextGoal='Course selesai. Nilai akhir akan menentukan badge Course Mastery.';
  }else{
    nextGoal='Semua milestone utama Course sudah tercapai.';
  }
...
function renderCourseGamification(data){
  var g=getCourseGamificationState(data);
  if(!g.totalLessons) return '';

  var cards=g.badges.map(function(b){
    var cls=b.unlocked?'achievement-card achievement-unlocked':'achievement-card achievement-locked';
    var status=b.unlocked
      ? '<span class="badge text-bg-success"><i class="bi bi-check-circle-fill me-1"></i>Terbuka</span>'
      : '<span class="badge text-bg-light"><i class="bi bi-lock-fill me-1"></i>Terkunci</span>';

    return '<div class="col-6 col-lg-4 col-xl-2">'+
      '<div class="'+cls+' h-100">'+
        '<div class="achievement-icon"><i class="bi '+b.icon+'"></i></div>'+
        '<div class="fw-bold small mt-2">'+esc(b.title)+'</div>'+
        '<div class="small text-secondary mt-1">'+esc(b.desc)+'</div>'+
  ...
  return '<div class="panel mb-3">'+
    '<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">'+
      '<div>'+
        '<h5 class="mb-1"><i class="bi bi-award-fill me-2 text-primary"></i>Achievement Course</h5>'+
        '<div class="small text-secondary">Badge hanya untuk motivasi belajar dan tidak mengubah nilai akademik.</div>'+
      '</div>'+
      '<div class="text-end">'+
        '<div class="small text-secondary">Badge Terbuka</div>'+
        '<div class="fw-bold fs-4">'+g.unlockedCount+'/'+g.badges.length+'</div>'+
      '</div>'+
    '</div>'+
    '<div class="row g-2">'+cards+'</div>'+
    '<div class="achievement-next-goal mt-3">'+
      '<i class="bi bi-signpost-2-fill me-2"></i><strong>Target berikutnya:</strong> '+esc(g.nextGoal)+
    '</div>'+
  '</div>';
}

function learningCelebrationKey(courseId){
  var userId=String(AppState.user&&AppState.user.id||'');
  return 'LMS_GAMIFICATION_V1::'+userId+'::'+String(courseId||'');
}

function checkLearningCelebration(data){
  if(!AppState.user||AppState.user.role!=='student') return;

  var course=data&&data.course?data.course:null;
  if(!course) return;

  var g=getCourseGamificationState(data);
  var key=learningCelebrationKey(course.id);
  var current={
    completeCount:g.completeCount,
    courseComplete:g.courseComplete,
    mastery:g.mastery,
    unlockedBadgeIds:g.badges.filter(function(b){return b.unlocked;}).map(function(b){return b.id;})
  };
  ...
  // First visit only records state; historical completions do not trigger a fake celebration.
  if(!previous){
  ...
  if(!previous.courseComplete && current.courseComplete){
    title='Course Selesai!';
    message='Seluruh Lesson pada '+String(course.title||'Course')+' sudah berhasil diselesaikan.';
  }else if(!previous.mastery && current.mastery){
    title='Course Mastery!';
    message='Nilai akhir Course mencapai minimal 85. Badge Course Mastery terbuka.';
  }else if(Number(current.completeCount||0)>Number(previous.completeCount||0)){
    title='Lesson Selesai!';
    message='Progres bertambah menjadi '+current.completeCount+' dari '+g.totalLessons+' Lesson.';
  }else{
    var prevIds=Array.isArray(previous.unlockedBadgeIds)?previous.unlockedBadgeIds:[];
    var newBadge=g.badges.find(function(b){
      return b.unlocked && prevIds.indexOf(b.id)===-1;
    });
    if(newBadge){
      title='Badge Baru!';
      message=newBadge.title+' berhasil dibuka.';
    }
  }

  try{localStorage.setItem(key,JSON.stringify(current));}catch(e3){}

  if(title) showLearningCelebration(title,message);
}

function showLearningCelebration(title,message){
  var old=document.getElementById('learningCelebrationOverlay');
  if(old) old.remove();

  var overlay=document.createElement('div');
  overlay.id='learningCelebrationOverlay';
  overlay.className='learning-celebration-overlay';
  overlay.innerHTML=
    '<div class="learning-celebration-card">'+
      '<div class="learning-celebration-icon"><i class="bi bi-trophy-fill"></i></div>'+
      '<h3 class="fw-bold mt-3">'+esc(title)+'</h3>'+
      '<p class="text-secondary mb-3">'+esc(message)+'</p>'+
      '<button class="btn btn-primary" onclick="closeLearningCelebration()">Lanjut Belajar</button>'+
    '</div>';
  ...
  for(var i=0;i<28;i++){
    ...
    piece.className='learning-confetti';
  ...
  setTimeout(function(){
    if(document.getElementById('learningCelebrationOverlay')){
      closeLearningCelebration();
    }
  },6000);
}

function closeLearningCelebration(){
  var el=document.getElementById('learningCelebrationOverlay');
  if(el) el.remove();
}
...
function selectLesson(id,button){
  ...
  var found=null;
  var lessons=(AppState.selectedCourse&&AppState.selectedCourse.lessons)||[];

  for(var i=0;i<lessons.length;i++){
    if(String(lessons[i].id)===String(id)){
      found=lessons[i];
      break;
    }
  }
  ...
  if(!found)return;
  if(found.locked){
    showToast('Lesson masih terkunci. Selesaikan Lesson sebelumnya terlebih dahulu.','warning');
    return;
  }
...
function renderLessonViewer(){
  ...
  if(!l){
    document.getElementById('lessonViewer').innerHTML=
      '<div class="text-center text-secondary py-5">Pilih Lesson untuk mulai belajar.</div>';
    return;
  }

  var header=
    '<div class="d-flex justify-content-between gap-3 mb-3">'+
      '<div><div class="small text-primary fw-semibold">LESSON</div><h2 class="fw-bold">'+esc(l.title)+'</h2></div>'+
      '<span class="badge text-bg-light align-self-start">#'+Number(l.order||1)+'</span>'+
    '</div>'+
    (l.description?'<p class="text-secondary">'+esc(l.description)+'</p>':'');
  ...
  if(l.locked){
    document.getElementById('lessonViewer').innerHTML=
      ...
      '<div class="alert alert-secondary py-4 text-center">'+
        ...
        '<strong>Lesson masih terkunci.</strong>'+
        '<div class="small mt-1">Selesaikan Lesson sebelumnya untuk membuka materi ini.</div>'+
      '</div>';
    return;
  }

  var progressSummary=renderLessonProgressSummary(l);

  var completedBanner=l.completed
    ? '<div class="alert alert-success"><i class="bi bi-trophy-fill me-2"></i><strong>Lesson selesai.</strong> Lesson berikutnya sudah terbuka.</div>'
    : '<div class="alert alert-primary py-2"><i class="bi bi-signpost-split me-2"></i>Tahap aktif: <strong>'+esc(flowStageLabel(l.next_stage))+'</strong></div>';

  var videoSection=renderFlowVideoSection(l);
  var textSection=renderFlowTextSection(l);
  var quizSection=renderFlowQuizSection(l);
  var taskSection=renderFlowTaskSection(l);

  document.getElementById('lessonViewer').innerHTML=
    header+progressSummary+completedBanner+videoSection+textSection+quizSection+taskSection;

  if(l.has_video && !l.video_completed){
    initVerifiedVideoTracker(l);
  }
}

function renderFlowVideoSection(l){
  var html=
    '<div class="border rounded-4 p-4 mb-4">'+
      '<div class="d-flex justify-content-between align-items-center mb-3">'+
        '<h5 class="mb-0"><span class="badge text-bg-primary me-2">1</span><i class="bi bi-play-btn me-2"></i>Video <span class="small text-secondary">'+(l.has_video?'('+formatLessonPoint((l.lesson_score||{}).video_max)+'%)':'(tanpa bobot)')+'</span> <span class="badge text-bg-light ms-2">'+(l.has_video?formatLessonPoint((l.lesson_score||{}).video_points)+'/'+formatLessonPoint((l.lesson_score||{}).video_max):'—')+'</span></h5>'+
        (l.video_completed?'<span class="badge text-bg-success"><i class="bi bi-check-circle me-1"></i>Selesai</span>':'')+
      '</div>';

  if(!l.has_video){
    return html+
      '<div class="alert alert-success mb-0">Lesson ini tidak memiliki video. Tahap Video dilewati otomatis.</div>'+
    '</div>';
  }

  var videoId=extractYoutubeVideoId(l.video_url);
  var direct=isDirectVideoUrl(l.video_url);

  if(videoId){
    if(l.video_completed){
      html+='<div class="video-wrap mb-3"><iframe src="https://www.youtube.com/embed/'+encodeURIComponent(videoId)+'?rel=0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>';
    }else{
      html+='<div class="video-wrap mb-3"><div id="verifiedYoutubePlayer"></div></div>';
    }
  }else if(direct){
    html+='<video id="verifiedHtml5Video" class="w-100 rounded-3 mb-3" controls preload="metadata" src="'+escAttr(l.video_url)+'"></video>';
  }else{
    html+=
      '<div class="alert alert-warning">'+
        '<strong>Video belum dapat diverifikasi otomatis.</strong><br>'+
        'Untuk sistem anti-skip gunakan URL YouTube atau file video langsung (MP4/WebM/Ogg). '+
        'Guru perlu memperbarui URL video pada Lesson ini.'+
      '</div>';
  }

  if(l.video_completed){
    html+='<div class="alert alert-success mb-0"><i class="bi bi-check-circle-fill me-2"></i>Video telah diselesaikan. Bobot Video <strong>30%</strong> sudah aktif.</div>';
  }else{
    var pct=Math.max(0,Math.min(100,Number(l.video_watch_percent||0)));
    html+=
      '<div class="mt-2">'+
        '<div class="d-flex justify-content-between small mb-1"><span>Progress tontonan terverifikasi</span><strong id="videoWatchProgressText">'+pct+'%</strong></div>'+
        '<div class="progress" style="height:10px"><div id="videoWatchProgressBar" class="progress-bar" style="width:'+pct+'%"></div></div>'+
        '<div class="small text-secondary mt-2"><i class="bi bi-info-circle me-1"></i>Bagian yang dilewati dengan seek tidak dihitung. Tidak ada tombol Tandai Selesai.</div>'+
      '</div>';
  }
...
function renderFlowTextSection(l){
  var locked=!l.video_completed;

  var html=
    '<div class="border rounded-4 p-4 mb-4">'+
      '<div class="d-flex justify-content-between align-items-center mb-3">'+
        '<h5 class="mb-0"><span class="badge text-bg-primary me-2">2</span><i class="bi bi-file-text me-2"></i>Materi Teks</h5>'+
        (l.text_completed?'<span class="badge text-bg-success"><i class="bi bi-check-circle me-1"></i>Selesai</span>':'')+
      '</div>';

  if(!l.has_text){
    return html+
      '<div class="alert alert-success mb-0">Lesson ini tidak memiliki materi teks. Tahap ini dilewati otomatis.</div>'+
    '</div>';
  }

  if(locked){
    return html+
      '<div class="alert alert-secondary mb-0"><i class="bi bi-lock-fill me-2"></i>Selesaikan Video terlebih dahulu untuk membuka Materi Teks.</div>'+
    '</div>';
  }

  html+='<div class="lesson-text">'+safeContent(l.content)+'</div>';

  if(!l.text_completed){
    html+=
      '<div class="mt-3">'+
        '<button class="btn btn-primary" onclick="completeTextStage()"><i class="bi bi-check2-circle me-1"></i>Saya Selesai Membaca Materi</button>'+
      '</div>';
  }else{
    html+='<div class="alert alert-success py-2 mt-3 mb-0">Materi teks sudah diselesaikan. Quiz terbuka.</div>';
  }

  return html+'</div>';
}

function renderFlowQuizSection(l){
  var unlocked=l.video_completed&&l.text_completed;

  var html=
    '<div class="border rounded-4 p-4 mb-4">'+
      '<div class="d-flex justify-content-between align-items-center mb-3">'+
        '<h5 class="mb-0"><span class="badge text-bg-primary me-2">3</span><i class="bi bi-patch-question me-2"></i>Quiz <span class="small text-secondary">'+(l.has_quiz?'(maks. '+formatLessonPoint((l.lesson_score||{}).quiz_max)+'%)':'(tanpa bobot)')+'</span> <span class="badge text-bg-light ms-2">'+(l.has_quiz?formatLessonPoint((l.lesson_score||{}).quiz_points)+'/'+formatLessonPoint((l.lesson_score||{}).quiz_max):'—')+'</span></h5>'+
        (l.quiz_completed?'<span class="badge text-bg-success"><i class="bi bi-check-circle me-1"></i>Sudah dikerjakan</span>':'')+
      '</div>';

  if(!l.has_quiz){
    return html+
      '<div class="alert alert-success mb-0">Lesson ini tidak memiliki Quiz. Tahap Quiz dilewati otomatis.</div>'+
    '</div>';
  }

  if(!unlocked){
    return html+
      '<div class="alert alert-secondary mb-0"><i class="bi bi-lock-fill me-2"></i>Selesaikan Video dan Materi Teks untuk membuka Quiz.</div>'+
    '</div>';
  }

  if(l.my_quiz_score!==null){
    html+='<div class="alert alert-success py-2">Nilai Quiz terbaik: <strong>'+Number(((l.lesson_score||{}).quiz_best)||l.my_quiz_score||0)+'</strong> · Kontribusi nilai: <strong>'+formatLessonPoint((l.lesson_score||{}).quiz_points)+'/'+formatLessonPoint((l.lesson_score||{}).quiz_max)+'</strong>. Tahap Tugas sudah terbuka.</div>';
  }
  ...
  for(var q=0;q<(l.quiz_questions||[]).length;q++){
    ...
    for(var o=0;o<(qq.options||[]).length;o++){
      optsArr.push(
        '<label class="d-block border rounded-3 p-2 mb-2">'+
          '<input type="radio" name="q'+q+'" value="'+o+'" class="me-2"> '+esc(qq.options[o])+
        '</label>'
      );
    }
    quizListArr.push(
      '<div class="mb-4">'+
        '<div class="fw-semibold mb-2">'+(q+1)+'. '+esc(qq.question||'Pertanyaan')+'</div>'+
        optsArr.join('')+
      '</div>'
    );
  }
  ...
  html+=
    '<div id="quizBox">'+quizListArr.join('')+
      '<button class="btn btn-primary" onclick="submitCurrentQuiz()">'+
        (l.quiz_completed?'Kerjakan Ulang':'Kirim Jawaban')+
      '</button>'+
    '</div>';
...
function renderFlowTaskSection(l){
  var unlocked=l.video_completed&&l.text_completed&&l.quiz_completed;
  var assignments=Array.isArray(l.assignments)?l.assignments:[];

  var html=
    '<div class="border rounded-4 p-4 mb-4">'+
      '<div class="d-flex justify-content-between align-items-center mb-3">'+
        '<h5 class="mb-0"><span class="badge text-bg-primary me-2">4</span><i class="bi bi-clipboard-check me-2"></i>Tugas <span class="small text-secondary">'+(l.has_task?'(maks. '+formatLessonPoint((l.lesson_score||{}).task_max)+'%)':'(tanpa bobot)')+'</span> <span class="badge text-bg-light ms-2">'+(l.has_task?formatLessonPoint((l.lesson_score||{}).task_points)+'/'+formatLessonPoint((l.lesson_score||{}).task_max):'—')+'</span></h5>'+
        (l.task_completed?'<span class="badge text-bg-success"><i class="bi bi-check-circle me-1"></i>Selesai dikumpulkan</span>':'')+
      '</div>';

  if(!l.has_task){
    return html+
      '<div class="alert alert-success mb-0">Lesson ini tidak memiliki Tugas. Tahap Tugas dilewati otomatis.</div>'+
    '</div>';
  }

  if(!unlocked){
    return html+
      '<div class="alert alert-secondary mb-0"><i class="bi bi-lock-fill me-2"></i>Kerjakan Quiz terlebih dahulu untuk membuka Tugas.</div>'+
    '</div>';
  }

  var asgArr=[];
  for(var a=0;a<assignments.length;a++){
    asgArr.push(renderAssignmentBlock(assignments[a]));
  }
  html+=asgArr.join('');

  if(l.task_completed){
    html+='<div class="alert alert-success mb-0"><i class="bi bi-unlock-fill me-2"></i>Semua Tugas sudah dikumpulkan. Lesson berikutnya terbuka tanpa harus menunggu penilaian Guru.</div>';
  }

  return html+'</div>';
}

function completeTextStage(){
  var l=AppState.selectedLesson;
  var course=AppState.selectedCourse&&AppState.selectedCourse.course;
  if(!l||!course)return;

  serverCall('markTextComplete',[AppState.user.id,course.id,l.id],{
    loading:'Menyimpan progress membaca...',
    success:function(res){
      if(!res.success){
...
function renderAssignmentBlock(a){
  ...
  var statusBadge=sub
    ? '<span class="badge text-bg-success ms-2">Sudah dikumpulkan</span>'
    : '<span class="badge text-bg-light ms-2">Belum dikumpulkan</span>';

  var gradeBadge=sub&&sub.score!==null&&sub.score!==''&&sub.score!==undefined
    ? '<span class="badge text-bg-primary ms-2">Nilai: '+sub.score+'</span>'
    : '<span class="badge text-bg-secondary ms-2">Belum dinilai</span>';

  var desc=a.description
    ? '<div class="small text-secondary mt-1">'+esc(a.description)+'</div>'
    : '';

  var submittedPreview=sub
    ? '<div class="small text-secondary mt-3 p-2 bg-light rounded">Terkumpul: <a href="'+escAttr(sub.file_url)+'" target="_blank">Lihat Tugas Saya</a> &mdash; '+esc(formatDateTime(sub.submitted_at))+'</div>'
    : '';
...
function submitCurrentQuiz(){
  ...
  serverCall('submitQuizFlow',[AppState.user.id,course.id,l.id,answers],{
    loading:'Menghitung dan menyimpan hasil Quiz...',
    success:function(res){
      if(!res.success){
        showToast(res.message,'danger');
        return;
      ...
      showToast('Nilai Quiz: '+score+'. Tahap Tugas sudah terbuka.','success');
      openCourse(course.id);
    }
  });
}
...
function ensureYoutubeApi(callback){
  ...
  window.onYouTubeIframeAPIReady=function(){
    ...
    var list=youtubeApiCallbacks.slice();
    youtubeApiCallbacks=[];
...
function initVerifiedVideoTracker(l){
  var course=AppState.selectedCourse&&AppState.selectedCourse.course;
  if(!course||!l||l.video_completed)return;

  var youtubeId=extractYoutubeVideoId(l.video_url);

  verifiedVideoTracker.courseId=String(course.id);
  verifiedVideoTracker.lessonId=String(l.id);
  verifiedVideoTracker.completed=false;
  verifiedVideoTracker.pending=false;

  if(youtubeId){
    verifiedVideoTracker.type='youtube';

    ensureYoutubeApi(function(){
      if(!document.getElementById('verifiedYoutubePlayer'))return;

      verifiedVideoTracker.player=new YT.Player('verifiedYoutubePlayer',{
        videoId:youtubeId,
        playerVars:{
          rel:0,
          playsinline:1,
        ...
        events:{
          onReady:function(){
            sendVerifiedVideoProgress(true);
          },
          onStateChange:function(ev){
            if(ev.data===YT.PlayerState.PLAYING){
              sendVerifiedVideoProgress(true);
              startVerifiedVideoTimer();
            }else{
              stopVerifiedVideoTimer();
  ...
  var video=document.getElementById('verifiedHtml5Video');
  if(video){
    verifiedVideoTracker.type='html5';
    verifiedVideoTracker.player=video;
...
function startVerifiedVideoTimer(){
  stopVerifiedVideoTimer();
  verifiedVideoTracker.timer=setInterval(function(){
    sendVerifiedVideoProgress(false);
  },4000);
}

function stopVerifiedVideoTimer(){
  if(verifiedVideoTracker.timer){
    clearInterval(verifiedVideoTracker.timer);
    verifiedVideoTracker.timer=null;
  }
}

function clearVerifiedVideoTracker(){
  stopVerifiedVideoTimer();

  try{
    if(verifiedVideoTracker.type==='youtube'&&verifiedVideoTracker.player&&verifiedVideoTracker.player.destroy){
      verifiedVideoTracker.player.destroy();
    }
  }catch(e){}

  verifiedVideoTracker.type='';
  verifiedVideoTracker.player=null;
  verifiedVideoTracker.courseId='';
  verifiedVideoTracker.lessonId='';
  verifiedVideoTracker.inFlight=false;
  verifiedVideoTracker.pending=false;
  verifiedVideoTracker.completed=false;
}

function verifiedVideoPosition(){
  try{
    if(verifiedVideoTracker.type==='youtube'&&verifiedVideoTracker.player){
      return {
        current:Number(verifiedVideoTracker.player.getCurrentTime()||0),
        duration:Number(verifiedVideoTracker.player.getDuration()||0)
      };
    }

    if(verifiedVideoTracker.type==='html5'&&verifiedVideoTracker.player){
      return {
        current:Number(verifiedVideoTracker.player.currentTime||0),
        duration:Number(verifiedVideoTracker.player.duration||0)
      };
    }
  }catch(e){}
...
function updateVerifiedVideoProgressUi(percent){
  var pct=Math.max(0,Math.min(100,Number(percent||0)));
  var bar=document.getElementById('videoWatchProgressBar');
  var text=document.getElementById('videoWatchProgressText');

  if(bar)bar.style.width=pct+'%';
  if(text)text.textContent=pct+'%';
}

function sendVerifiedVideoProgress(force){
  if(verifiedVideoTracker.completed)return;
  if(!verifiedVideoTracker.courseId||!verifiedVideoTracker.lessonId)return;

  if(verifiedVideoTracker.inFlight){
    if(force)verifiedVideoTracker.pending=true;
    return;
  }

  var pos=verifiedVideoPosition();
  if(!pos.duration||pos.duration<=0)return;

  verifiedVideoTracker.inFlight=true;

  LMSApi.call('recordVideoProgress',[
    AppState.user.id,
    verifiedVideoTracker.courseId,
    verifiedVideoTracker.lessonId,
    pos.current,
  ...
  .then(function(res){
    ...
    var data=res.data||{};
    updateVerifiedVideoProgressUi(data.watched_percent||0);

    if(data.completed){
      verifiedVideoTracker.completed=true;
      stopVerifiedVideoTimer();

      applyProgressUpdate(data.progress_by_course||{});
      showToast('Video selesai. Bobot Video 30% diperoleh dan Materi Teks terbuka.','success');

      var cid=verifiedVideoTracker.courseId;
      setTimeout(function(){
        openCourse(cid);
      },400);
    }
  ...
  .catch(function(){
    // Ping video dibuat senyap agar tidak mengganggu proses menonton.
    // Ping berikutnya akan mencoba menyimpan lagi.
  })
  .finally(function(){
    verifiedVideoTracker.inFlight=false;

    if(verifiedVideoTracker.pending&&!verifiedVideoTracker.completed){
      verifiedVideoTracker.pending=false;
      setTimeout(function(){
        sendVerifiedVideoProgress(false);
      },100);
